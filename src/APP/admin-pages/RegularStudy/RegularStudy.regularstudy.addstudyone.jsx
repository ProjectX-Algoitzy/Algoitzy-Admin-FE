...
export default function RegularStudyAddStudyone() {
    ...
    const handleAddMember = async (memberId) => {
        ...
        if (confirmation) {
            try {
                const response = await request.post(`/study/${id}/study-member`, {
                    memberId,
                });
                if (response.isSuccess) {
                    await alert('추가되었습니다.');
    ...
    return (
        <itemS.Container>
            ...
            <itemS.IntroduceContainer>
                ...
                <itemS.IntroduceSentence>
                    정규 스터디를 진행 중이거나, 정규 스터디를 수료한 이력이 있는 회원만 노출됩니다.
                </itemS.IntroduceSentence>
            </itemS.IntroduceContainer>
            ...
            <itemS.StyledTable>
                <thead>
                    <tr>
                        <itemS.StyledTd rowIndex={0} colIndex={0}>
                            이름
                        </itemS.StyledTd>
                        <itemS.StyledTd rowIndex={0} colIndex={1}>
                            백준 닉네임
                        </itemS.StyledTd>
                        <itemS.StyledTd rowIndex={0} colIndex={2}>
                            학과
                        </itemS.StyledTd>
                    ...
                    {members.map((member, index) => (
                        <tr key={member.memberId}>
                            ...
                            <itemS.StyledTd rowIndex={index + 1} colIndex={3} style={{textAlign: 'right'}}>
                                <itemS.AcceptanceBtn onClick={() => handleAddMember(member.memberId)}>
                                    추가
                                </itemS.AcceptanceBtn>
                            </itemS.StyledTd>
                        </tr>
