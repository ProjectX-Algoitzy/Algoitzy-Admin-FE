...
export const StudySelectContainerArbitary = styled(Select).attrs({
  ...
  .react-select__menu {  /*클릭 시 나오는 드롭다운 메뉴의 스타일을 지정*/
    ...
    left: -1px;
    width: 212px;
    max-height: 298px; // 메뉴의 최대 높이를 설정 
    border-radius: 4px;
    border: none;
...
export const QuestionContainer = styled.input`
  ${({ innerContainerClicked }) => 
    ...
    `: css`
        width: ${({ value }) => (value ? `${value.length * 14.2 + 10}px` : '460px')}; /* 약간의 여유 추가 */ /*기존코드: width: ${({ value }) => (value ? `${value.length * 14.2}px` : '460px')}; */
        min-width: 36px;
        margin-right: 16px;
        height: auto;
        border: none;
        ${tokens.typography.T5_SB_16};
        letter-spacing: 0.5px; /* 글자 간 간격 추가 */
  `}
`;
