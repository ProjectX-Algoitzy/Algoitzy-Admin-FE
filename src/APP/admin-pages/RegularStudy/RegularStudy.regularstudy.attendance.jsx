    ...
    const handleIconClick = (rowIndex, colIndex) => {
        ...
        if (React.isValidElement(currentIcon) && currentIcon.props && currentIcon.props.src) {
            ...
            const attendanceType =
                currentTab === '문제 인증' ? 'PROBLEM' : currentTab === '블로그 포스팅' ? 'BLOG' : 'WORKBOOK';
            ...
            if (
                ...
                const attendanceIndex = updatedAttendanceList[currentTab].findIndex(
            ...
            const allIconsUnmodified = Object.values(updatedData).every((tab) =>
                tab.every((row) =>
                    row.every(
                        (cell) => !React.isValidElement(cell) || !cell.props.src || !cell.props.src.includes('check')
                    )
                )
            );
