    ...
    if (confirmation) {
      try {
        const response = await request.delete(
          `/workbook/${workbookId}/problem/${problemNumber}`
        );
        if (response.isSuccess) {
          setWeekData((prevData) => {
            const updatedWeekData = { ...prevData };
            updatedWeekData[week] = updatedWeekData[week].filter(
              (question) => question.id !== problemNumber.toString()
            );
    ...
    <itemS.Container>
      ...
      {hasWeekData ? (
        <>
              ...
              {weekData[week].map((row, index) => (
                <itemS.TableRow key={index}>
                  ...
                  <itemS.TableCell>
                    <itemS.DeleteIcon
                      onClick={() =>
                        handleDeleteQuestion(row.workbookId, row.id)
                      }
                      alt="cancel"
                    />
                  </itemS.TableCell>
          ...
          <itemS.BtnContainer>
            <itemS.AddQuestionBtn onClick={handleOpenModal}>
              문제 추가하기
            </itemS.AddQuestionBtn>
          </itemS.BtnContainer>
        </>
      ) : (
        <>
      ...
      {isModalOpen && (
        <AddQuestionModal
          ...
          onAddQuestion={(id, title, levelImg) =>
            handleAddQuestion(id, title, levelImg)
          }
        />
      )}
    </itemS.Container>
