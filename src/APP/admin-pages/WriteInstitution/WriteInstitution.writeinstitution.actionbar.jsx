...
import * as Styled from './Styled/WriteInstitution.writeinstitution.actionbar.styles';
import request from '../../Api/request';
import DraftModal from './WriteInstitution.writeinstitution.draft';
  ...
  const handlePostSubmit = async () => {
    ...
    try {
      let response;
        if (boardId) {
          // 게시글 수정(임시저장 글 포함)
          response = await request.patch(`/institution/${boardId}`, requestData);
        } else {
          // 새 게시글 작성
          response = await request.post('/institution', requestData);
        }
        if (response.isSuccess) {
